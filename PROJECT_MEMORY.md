...
## 19. 最新进展（2026-02-21 凌晨 6）
- 按用户确认继续完成 1/2/3：
  - 新增上线配置文档：`公众号上线配置项清单.md`
  - 新增数据库补丁脚本：`smart_minimal_patch.sql`
  - 新增本地启动文档：`本地启动指南_Mac.md`
- 目标保持不变：先功能上线，再优化。

## 20. 最新进展（2026-02-21 凌晨 7）
- 按用户确认继续完成 1/2/3：
  - 生产配置落地：`AuthController` 支持环境变量兜底并增强字段完整性；
  - 入口参数校验：`access/getToken` 增加必填校验与可读错误提示；
  - 冒烟自动化：新增 `smoke_api.sh` 一键检查关键接口 PASS/FAIL。
