...
(function (w) {
  ...
  var _scriptPath = (function (func) {
    ...
    _js = _js[_js.length - 1].src.substring(
      0,
      _js[_js.length - 1].src.lastIndexOf('/') + 1
    )
    func(_js)
    return _js
  })(function (s) {
    var _buildLib = '<link rel="stylesheet" href="./EleditorLayout.css">'
  ...
  var _correctHtmlStructure = function () {
    var _$wrap = arguments[0];
      var _empty = arguments[1]

    if (
      _formatInnerText(_$wrap.text()) == '' &&
      _$wrap.find('img').length === 0
    ) {
  ...
  var _buildEditorModule = function (_toolbars, _uid) {
    var _html =
      '<div class="Eleditor-wrap" style="z-index:' +
      _getLayerMaxZIndex() +
      '" id="' +
      _uid +
      '"><div class="Eleditor-controller"><ul>'

    for (var i = 0; i < _toolbars.length; i++) {
      var _it = _toolbars[i];
        var _id = typeof _it === "object" ? _it.id : _it;
        var _tag =
          typeof _it === "object" && _it.tag
            ? _it.tag.toLocaleLowerCase()
            : null;
        var _name = typeof _it === 'object' ? _it.name : _toolnames[_it]
      _html +=
        '<li event="' +
        _id +
        '" ' +
  ...
  w[_namespace] = function () {
    ...
    (_placeHolder =
        _arg.placeHolder ||
        '<p class="Eleditor-placeholder" style="color: #999">点击此处编辑内容</p>')
    ...
    if (_arg.el instanceof $) {
      ...
      if (_$wrap.length === 0) {
        return console.warn(
          '|--Eleditor ' +
            _arg.el +
            '元素不存在，请在DOMContentLoaded后初始化Eleditor'
        )
    ...
    var _$editorTextArea = _$editorTextModule.find(
      '.Eleditor-inputarea .textarea'
    )
    var _$editorUploadImageBtn = _$editorController.find(
      '.Eleditor-insertImage'
    )
    var _$editorTextLinkArea = _$editorTextModule.find(
      '.Eleditor-inputarea input'
    )
    var _$editorColorModule = _$editorTextModule.find(
      '.Eleditor-textEditor-colors'
    )
    var _$editorFontsizeModule = _$editorTextModule.find(
      '.Eleditor-textEditor-fontsizes'
    )
    var _$editorUndoBtn = _$editorWrap.find('.Eleditor-undo')
    var _$editorLineheightModule = _$editorTextModule.find(
    ...
    var _showEditorControllerLayer = function (_$e) {
      ...
      $.each(_$editorController.find('li'), function (i, e) {
        var _$e = $(e);
          var _tg = _$e.attr('on-tag')
        if (_tg) {
          _tg = _tg.split(',')
          var _sel = _$selected[0].tagName.toLocaleLowerCase()
          if (_sel == _tg[0] || _sel == _tg[1]) {
            _$e.show()
          } else {
            _$e.hide()
          }
    ...
    var _handleHistory = function () {
      ...
      if (_handle.m == 'editNode') {
        _handle.node
          .attr('style', _handle.unode.attr('style') || '')
          .html(_handle.unode)
      }
    ...
    var _syncRenderTextEditorView = function () {
      ...
      if (_$selected.css('font-weight') == 'bold') {
        _$editorTextModule
          .find('.Eleditor-textStyle-bold')
          .addClass('Eleditor-active')
      }
      if (_$selected.css('text-decoration') == 'underline') {
        _$editorTextModule
          .find('.Eleditor-textStyle-underline')
          .addClass('Eleditor-active')
      }
      ...
      if (_inArray(_selectAlign, ['left', 'center', 'right'])) {
        _$editorTextModule
          .find('.Eleditor-textStyle-align[align=' + _selectAlign + ']')
          .addClass('Eleditor-active')
      } else {
        _$editorTextModule
          .find('.Eleditor-textStyle-align')
          .removeClass('Eleditor-active')
      }
      _$editorTextModule
        .find('.Eleditor-textStyle-color span')
        .css('background-color', _$selected.css('color'))
    };
    ...
    if (_inArray('insertImage', _arg.toolbars)) {
      if (typeof WebUploader !== 'undefined') {
        _uploader = WebUploader.create({
          ...
          duplicate: true,
          compress:
            typeof _arg.upload.compress === 'undefined'
              ? false
              : _arg.upload.compress,
          resize: false,
          fileSingleSizeLimit: _arg.upload.fileSizeLimit
            ? _arg.upload.fileSizeLimit * 1024 * 1024
            : undefined,
          accept: {
            title: 'Images',
    ...
    var _editorModuleEvents = {
      ...
      insertHr: function () {
        var _$hr = $(
          '<div class="horizontal-line" style="padding: 10px 0;border-bottom: 1px solid #aaa;margin-bottom: 20px;"></div>'
        )
    ...
    for (var i = 0; i < _arg.toolbars.length; i++) {
      if (typeof _arg.toolbars[i].handle === 'function') {
    ...
    _$editorTextModule.on('click', '.Eleditor-textStyle-bold', function () {
      _$editorTextArea.css(
        'font-weight',
        $(this).hasClass('Eleditor-active') ? 'normal' : 'bold'
      )
    ...
    _$editorTextModule.on('click', '.Eleditor-textStyle-underline', function () {
      _$editorTextArea.css(
        'text-decoration',
        $(this).hasClass('Eleditor-active') ? 'none' : 'underline'
      )
      $(this).toggleClass('Eleditor-active')
    })
    _$editorTextModule.on(
      'click',
      '.Eleditor-textStyle-color,.Eleditor-textStyle-bgColor',
      function () {
        var _$this = $(this)
        var _role = _$this.hasClass('Eleditor-textStyle-bgColor')
          ? 'bgcolor'
          : 'color';
        _$editorColorModule
          .find('.Eleditor-textEditor-modulePane span')
          .html(_role == 'bgcolor' ? '文字背景颜色' : '文字颜色')
    ...
    _$editorLineheightModule.on('click', 'ul li', function () {
      if (!$(this).hasClass('Eleditor-inheritValue')) {
        ...
        _$editorTextModule
          .find('.Eleditor-textStyle-lineHeight')
          .removeClass('Eleditor-active')
      }
    ...
    _$editorFontsizeModule.on('click', 'ul li', function () {
      if (!$(this).hasClass('Eleditor-inheritValue')) {
        ...
        _$editorTextModule
          .find('.Eleditor-textStyle-fontSize')
          .removeClass('Eleditor-active')
      }
    ...
    _$editorTextModule.on('click', '.Eleditor-textStyle-align', function () {
      ...
      _$editorTextModule
        .find('.Eleditor-textStyle-align.Eleditor-active')
        .removeClass('Eleditor-active')
      $(this).addClass('Eleditor-active')
    })
    ...
    _$editorTextModule.on(
      'click',
      '.Eleditor-cancel,.Eleditor-commit',
      function () {
        if ($(this).hasClass('Eleditor-commit')) {
          ...
          if (!_content) {
            return alert(
              '这里不能留空喔，请输入内容文字！如不需要可以选择删除'
            )
          }
          if (_$editorTextModule.attr('role') == 'edit') {
            if (_$editorTextModule.attr('type') == 'link') {
              var _link = _$editorTextLinkArea.val()
              _$selected.attr('href', _link)
            }
            _$selected.attr('style', _style).html(_content)
            ...
            _$selected.before(_buildWordHtml)
            _flushEditorControllerLayerPosi()
    ...
    _$editorColorModule.on('click', 'ul li span', function () {
      ...
      if (_$editorColorModule.attr('role') == 'color') {
        ...
        if (!$(this).hasClass('Eleditor-inheritValue')) {
          ...
          _$editorTextModule
            .find('.Eleditor-textStyle-bgColor')
            .removeClass('Eleditor-active')
        }
      }
