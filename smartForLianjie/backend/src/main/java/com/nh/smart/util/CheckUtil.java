 ...
 * 
 * @ClassName: CheckUtil
 * @Description: 参数校验，判断工具类
 * 				 表达校验过程中，不会对式进行校验，所以请确保表达式的正确性
 * @Author Demo
 * @DateTime 2020年2月1日 下午3:56:22
 */
public class CheckUtil {
	...
	/**
	 * 判断参数是否是一个日期 支持Date类型 支持LocalDate类型
	...
	/**
	 * 判断参数是否是一个日期 支持Date类型 支持LocalDateTime类型 支持String类型，yyyy-MM-dd
	 * HH:mm:ss、yyyyMMddHHmmss、yyyy/MM/dd HH:mm:ss格式； 默认仅支持yyyy-MM-dd HH:mm:ss
	 */
	public static Boolean isDateTime(Object value, String express) {
		...
		if (value instanceof String) { // 通常json格式参数，都是以字符串类型传递，优先判断
			...
			try {
				...
				/*
				 * try { LocalDateTime.parse(v, DateTimeFormatter.ofPattern("yyyyMMddHHmmss"));
				 * return Boolean.TRUE; } catch (Exception e1) { return Boolean.FALSE; }
				 */
				return Boolean.FALSE;
			}
		}
	...
	/**
	 * 判断参数是否是一个过去时间 支持Date类型 支持LocalDate类型 支持LocalDateTime类型
	 * 支持String类型，yyyy-MM-dd、yyyyMMdd、yyyy/MM/dd格式； 默认仅支持yyyy-MM-dd
	 * 支持String类型，yyyy-MM-dd HH:mm:ss、yyyyMMddHHmmss、yyyy/MM/dd HH:mm:ss格式；
	 * 默认仅支持yyyy-MM-dd HH:mm:ss
	 * 
	 * @param value
	 * @param express
	 * @return
	...
	public static Boolean isPast(Object value, String express) {
		...
		if (value instanceof String) { // 通常json格式参数，都是以字符串类型传递，优先判断
			...
			try {
				...
				/*
				 * try { LocalDateTime ldt = LocalDateTime.parse(v,
				 * DateTimeFormatter.ofPattern("yyyyMMddHHmmss")); return
				 * LocalDateTime.now().isAfter(ldt); } catch (Exception e1) { return
				 * Boolean.FALSE; }
				 */
				return Boolean.FALSE;
			}
	...
	/**
	 * 判断参数是否是一个将来时间 支持Date类型 支持LocalDate类型 支持LocalDateTime类型
	 * 支持String类型，yyyy-MM-dd、yyyyMMdd、yyyy/MM/dd格式; 默认仅支持yyyy-MM-dd
	 * 支持String类型，yyyy-MM-dd HH:mm:ss、yyyyMMddHHmmss、yyyy/MM/dd HH:mm:ss格式；
	 * 默认仅支持yyyy-MM-dd HH:mm:ss
	 * 
	...
	public static Boolean isFuture(Object value, String express) {
		...
		if (value instanceof String) { // 通常json格式参数，都是以字符串类型传递，优先判断
			...
			try {
				 ...
				 * try { LocalDateTime ldt = LocalDateTime.parse(v,
				 * DateTimeFormatter.ofPattern("yyyyMMddHHmmss")); return
				 * LocalDateTime.now().isBefore(ldt); } catch (Exception e1) { return
				 * Boolean.FALSE; }
				 */
				return Boolean.FALSE;
			}
	...
	/**
	 * 判断参数是否在枚举的数据中, 枚举的表达式用 英文逗号隔开，无空格，如： "男,女,太监" 校验过程，不在对表达式进行校验，所以请确保表达式的格式正确
	 * 支持String 支持Integer Short Long
	 * 
	 * @param value
	 * @param enumStr
	 * @return
	...
	/**
	 * 是否大于指定值 支持Integer、Long、Short、Float、Double、BigDecimal 支持String，判断length值
	...
	/**
	 * 是否大于等于 支持Integer、Long、Short、Float、Double、BigDecimal 支持String，判断length值
	 * 支持Collection，判断size的值
	 * 
	 * @param value
	 * @param express
	...
	/**
	 * 是否少于 支持Integer、Long、Short、Float、Double、BigDecimal 支持String，判断length值
	 * 支持Collection，判断size的值
	 * 
	 * @param value
	 * @param express
	...
	/**
	 * 是否少于等于 支持Integer、Long、Short、Float、Double、BigDecimal 支持String，判断length值
	 * 支持Collection，判断size的值
	 * 
	 * @param value
	 * @param express
	...
	/**
	 * 判断是否 notEqual指定的值 支持String、Integer、Long、Short、Float、Double、BigDecimal
	 * 支持Collection，判断size的值
	 * 
	 * @param value
	 * @param express
	...
	/**
	 * 判断是否Equal指定的值 支持String、Integer、Long、Short、Float、Double、BigDecimal
	 * 支持Collection，判断size的值
	 * 
	 * @param value
	 * @param express
