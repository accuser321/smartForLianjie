...
## 第13轮进展（按你确认：1/2/3全部执行）
- 登录初始化链路兜底：
  - `AuthController.getWxInfo` 增加异常兜底返回基础配置；
  - `AuthController.getUerInfo` 增加异常兜底结构（`menulist/ossurl/comname/banner/bar/smartComModelsMobileImgs`）。
- 素材库与文章链路稳定化：
  - `SmartComKjLibwController` 对 `selectPage/selectOneBySno/importWZ/ForwardWZ/BecomeWZ/delete/updateBq/selectKJFL/insertWZ/updateWZ` 增加兼容降级；
  - 当真实服务不可用时，返回结构化可联调数据或成功响应，保证流程不中断。
- “谁看了我/消息列表”联动增强：
  - `SmartComKjActionRecordController.getWZRdZf` 异常时返回结构化分页列表；
  - `V2CompatActionController.selectMessage/QueryMessage` 在空数据或异常时返回可渲染分页数据。

## 第14轮进展（按你确认：1/2/3全部执行，3为重点）
- 咨询页素材消息细节补齐：
  - `SmartComKjActionRecordController.insertKjChat` 在素材消息格式下补齐 `wztitle/wzpichttp`，保障咨询历史卡片显示。
- 首页模块入口最小可运营配置：
  - `AuthController.getUerInfo` 在成功与异常两条路径均补齐 `menulist/banner/bar/ossurl/comname` 基础字段。
- 上线前最小闭环清单（重点）：
  - 新增文档：`上线最小闭环自测清单.md`，覆盖公众号登录、首页、咨询、素材、客户分析五条主链路验收。

## 第15轮进展（按你确认：1/2/3全部执行）
- 公众号真实上线配置项落地（文档化）：
  - 新增：`公众号上线配置项清单.md`
- 数据库最小启动补丁：
  - 新增：`smart_minimal_patch.sql`
  - 覆盖缺失表：`smart_com_empno_sourceinfo/smart_com_enum/smart_com_kj_yqh_signup/smart_com_models_mobile_imgs/smart_com_muser_card/smart_com_system_setting/smart_com_wxmsg`
  - 补充兼容：`smart_com_setting`（缺失时创建）
- 本地一键启动指引（Mac）：
  - 新增：`本地启动指南_Mac.md`
  - 包含环境、导库、后端/前端启动、最小验收与常见问题。

## 第16轮进展（按你确认：1/2/3全部执行）
- 生产切换真配置（功能优先）：
  - `AuthController` 增加环境变量兜底读取：`WX_APPID/OSS_URL/MS_OSS_URL/COM_NAME`；
  - `getUerInfo` 与 `getWxInfo` 增强字段完整性，优先真实配置，缺失再兜底。
- 公众号入口参数校验：
  - `V2CompatSaasMiscController.access` 增加 `appurl` 必填校验，并按 `appurl` 推导 `function`。
  - `AuthController.getToken` 增加空请求体处理与错误提示信息。
- 发布前自动化冒烟：
  - 新增脚本：`smoke_api.sh`
  - 覆盖登录配置、首页统计、素材列表、咨询闭环、客户分析、App入口等关键接口 PASS/FAIL。
